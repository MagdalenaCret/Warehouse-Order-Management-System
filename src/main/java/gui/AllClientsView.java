...
public class AllClientsView extends JFrame {
    ...
    public void displayClients(List<Client> clients, ActionListener viewAction, ActionListener deleteAction) {
        viewButtons = new ArrayList<>();
        deleteButtons = new ArrayList<>();

        String[] columns = new String[]{"Gender", "First Name", "Last Name", "Phone Number", "Email", "View", "Delete"};
        ...
        for (Client client : clients) {
            ...
            {
                data[index][0] = getWomanImage();
            }else{
                data[index][0] = getManImage();
            }

            data[index][1] = client.getFirstName();
            data[index][2] = client.getLastName();
            data[index][3] = client.getNumberOfPhone();
        ...
        this.clientsTable.setModel(new DefaultTableModel(data, columns));
        ...
        this.clientsTable.getColumnModel().getColumn(0).setCellRenderer(buttonRendered);


        // Text
        this.clientsTable.getColumnModel().getColumn(1).setCellRenderer(textRendered);
        this.clientsTable.getColumnModel().getColumn(2).setCellRenderer(textRendered);
        this.clientsTable.getColumnModel().getColumn(3).setCellRenderer(textRendered);
        this.clientsTable.getColumnModel().getColumn(4).setCellRenderer(textRendered);

        // Buton
        this.clientsTable.getColumnModel().getColumn(5).setCellRenderer(buttonRendered);
        this.clientsTable.getColumnModel().getColumn(6).setCellRenderer(buttonRendered);


        // Sterg ce era inainte pentru a regenera logica in caz de refresh
        for (MouseListener mouseListener : this.clientsTable.getMouseListeners()) {
            this.clientsTable.removeMouseListener(mouseListener);
        }

        this.clientsTable.addMouseListener(new TableMouseListener(data, this.clientsTable.getRowHeight(), buttonIndexes, this.clientsTable));
    ...
    private JButton getWomanImage() {
        ...
        jButton2.setContentAreaFilled(false);
        jButton2.setDefaultCapable(false);
        jButton2.setIcon(new ImageIcon(getClass().getResource("/woman_table.png")));
        jButton2.setText("");
        return jButton2;
    }

    private JButton getManImage() {
        ...
        jButton2.setContentAreaFilled(false);
        jButton2.setDefaultCapable(false);
        jButton2.setIcon(new ImageIcon(getClass().getResource("/man_table.png")));
        jButton2.setText("");
        return jButton2;
    }
    ...
    private void init() {
        ...
        clientsTable.setBackground(new Color(-12106425));
        Font table1Font = this.getFont("Consolas", Font.PLAIN, 16, clientsTable.getFont());
        if (table1Font != null) clientsTable.setFont(table1Font);
        clientsTable.setForeground(new Color(-3618098));
        clientsTable.setGridColor(new Color(-12106425));
        clientsTable.setOpaque(false);
        clientsTable.setRowHeight(75);
        clientsTable.setSelectionBackground(new Color(-8421505));
        clientsTable.setSelectionForeground(new Color(-1738426));
        ...
        addButton.setIcon(new ImageIcon(getClass().getResource("/button_add.png")));
        addButton.setText("");
