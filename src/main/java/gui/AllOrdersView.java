...
public class AllOrdersView extends JFrame {
    ...
    public void displayOrders(List<OrderView> orderViews, ActionListener deleteAction) {
        deleteButtons = new ArrayList<>();

        String[] columns = new String[]{"Gender", "Order Quantity", "Date", "Price", "Delete"};
        List<Integer> buttonIndexes = new ArrayList<>();
        buttonIndexes.add(4);

        Object[][] data = new Object[orderViews.size()][columns.length];
        int index = 0;
        for (OrderView orderView : orderViews) {

            data[index][0] = getOrderImage();
            data[index][1] = orderView.getProductName();
            data[index][2] = orderView.getQuantity() + " items";
            data[index][3] = orderView.getOrderPrice() + " $";

            JButton deleteButton = createDeleteButton();
            deleteButton.addActionListener(deleteAction);
            data[index][4] = deleteButton;
        ...
        this.ordersTable.getColumnModel().getColumn(0).setCellRenderer(buttonRendered);
        ...
        // Buton
        this.ordersTable.getColumnModel().getColumn(4).setCellRenderer(buttonRendered);


        // Sterg ce era inainte pentru a regenera logica in caz de refresh
        for (MouseListener mouseListener : this.ordersTable.getMouseListeners()) {
            this.ordersTable.removeMouseListener(mouseListener);
        }

        this.ordersTable.addMouseListener(new TableMouseListener(data, this.ordersTable.getRowHeight(), buttonIndexes, this.ordersTable));
    ...
    private JButton getOrderImage() {
        ...
        jButton2.setContentAreaFilled(false);
        jButton2.setDefaultCapable(false);
        jButton2.setIcon(new ImageIcon(getClass().getResource("/orders.png")));
        jButton2.setText("");
        return jButton2;
    }
    ...
    private void init() {
        ...
        addButton.setIcon(new ImageIcon(getClass().getResource("/add_order.png")));
        addButton.setText("");
